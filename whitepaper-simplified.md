...
### 以太坊和Solidity的常见的漏洞
区块链技术所采用的Nick Szabo的智能合约的是一个计算机程序，它会按严格程序运转不受任何权威机构的操控。
Ethereum协议仅支持有效合约，这就可以筛选出需要进行操作的地址。合约仅能接受来自既定用户地址的操作。
如果这样的交易被区块链接受，那么网络的所有参与者将执行这个合约代码。网络就会对合约的输出和下一个阶段的发展达成一致。由于以太坊智能合约是不可改变的，交易结果是不可逆转的，所以在发布之前有效地解释代码的含义十分的重要。
Atzei等人在《Solidity for Ethereum》中描述了智能合约的缺陷分类和会产生的异常行为。尽管现在只有以太坊有这种缺陷，但是未来使用以太坊合约的其他平台也很快会面临如何解决这种缺陷。我们根据用户的发现对这些缺陷做了如下归纳。


&nbsp;| &nbsp;
---|---
对于未知函数的调用 | 一些坚实的原语有非明显的副作用调用收件人的回退函数。这可能导致意外行为，可能被攻击者利用。(我们在关于奇偶校验/多重集漏洞的部分中讨论此问题。
异常处理| 对于如何处理异常, 有两种不同的方法，采取哪一种主要取决于合同与项目的关联性。有的异常处理可以恢复所有的异常；而有的异常处理仅仅只能恢复智能合同被调用后的结果。因为如果进行更多修正将会影响到原有合同的安全性。
无Gas数据传输 | 当用户向合同发送以太坊时, 可能会出现不能进行gas传输的异常。
