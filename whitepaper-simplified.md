...
软件验证带来了许多好处：更好的代码，更好的测试，更少的黑客，并且是提高软件安全性的有效方法。 SAT 求解器是这项工作的重要工具。 在本节中，我们对SAT和Parallel SAT进行了粗略的讨论。
...
SAT的一个问题实例由n个变量中的布尔公式f组成。SAT求解器决定一个令人满意的变量赋值的存在与否；换言之，对每个变量的赋值为真或假，使公式本身为真。大多数解算符要求f是在合法线形式 (CNF) 中指定的，其中包括公式由子句连接组成，每个词句由一个词的分离组成10。
典型的 SAT-solver11 通过实际的求解过程，从问题的高级编码中参与以下三步工作流。
1. 编码  
...
通常依赖SAT-Solvers的模型检查和自动定理证明的技术，可能需要从毫秒到小时的计算工作商品的机器。对于一些最难解决的问题，解决时间可以延长到几天，几周，或更长时间。最近在算法、启发式和并行求解方面的进步是有帮助的。能够分担工作的人比那些没有工作的人表现更好。平行的 SAT Solver试图使用更多的内核来克服连续的减速。

一个典型的并行 SAT-solver使用主从 (或任务场) 方法，分割搜索空间并在单独的过程中并行分析子空间。这是个很好的例子方法是并行的微型SAT13 (PMSAT)。PMSAT 是一个分布式并行 SAT-solver，使用消息传递接口 (MPI) 在 C++ 中实现节点.PMSAT在以下方面是新颖的: (1) 它如何划分搜索空间可变选择和假设生成；(2) 假设是如何被修剪的；(3) 如何学会子句共享；和 (4) 自动设置。主控制客户的调度并在它们之间分配各种任务。可以使用多个分区启发式允许用户使用和共享学习条款。冲突学习用于修剪出色的任务，并可能停止其搜索空间不相关的进程。两个可选变量选择: (1) 频繁变量，或 (2) 变量出现在更大的条款中。
...
另一种平行 SAT-sovler采用组合方法；也就是说，它依赖于运行在同一 SAT 实例中，多个解决方案并行。这项技术是最先进的并行 SAT-solver，目前由 ManySAT14 控制.很多ManySAT已发现使用不同的搜索启发式，甚至是不同的 SAT-solver，在性能上带来了巨大的收益。通过共享学习的条款，在不同的实例中可以观察到性能的提高。
...
#### 平行 SAT 和共识
如前所述，关于 Quantstamp 的验证协议，分布式计算是将其划分成整体验证问题的组成部分。这是一个重要的机制为抑制坏演员。将这个返回到分布式设置中，在搜索空间分区，每个分区，或子空间，是由一个序列的 SAT 规划求解。在Quantstamp 网络，如果可能，不相交子空间映射到节点的分区称为区域。在每个区域中，节点工作，以找到一个令人满意的分配在一个离散空间.由于分区是分离的，所以在一个区域内识别出一个令人满意的任务对原公式有一个令人满意的赋值。第一步的编码过程确保在公式满足时，原始系统中存在一个错误。最终区域的输出需要参与分区 2/3的一致意见，一般需要多数人达成一致意见。

### 以太坊和Solidity的常见的漏洞
Nick Szabo的“智能合约”的区块链实现是一个计算机程序。在不依赖可信授权的情况下执行正确的执行。以太坊协议支持有状态的契约，这意味着状态变量的值会持续存在多个调用。当它接收来自用户的事务时，就会调用一个契约地址。
如果这样的交易被区块链所接受，那么挖掘网络的所有参与者合约执行的代码。然后，网络通过共识协议，然后在合约的下一阶段，就输出结果达成一致。鉴于以太坊的智能合约是不可变的交易是不能逆转的，重要的是足够有效得推理在部署之前的代码。
Atzei 等人17 描述了智能漏洞的分类和智能的意外行为为 以太坊而写的合约。虽然此分类法特定于以太坊，但它是将来使用合约的其他平台，可能会存在类似的漏洞。我们根据他们的发现，对下面的分类法进行总结。


&nbsp；| &nbsp；
---|---
调用未知 | 一些坚实的原语有非明显的副作用调用收件人的回退函数。这可能导致意外行为，可能被攻击者利用。(我们在关于奇偶校验/多重集漏洞的部分中讨论此问题。
异常无序 | 对于如何处理异常，有两种不同的行为，这取决于合同如何项目联系。对于一些人来说，整个交易的副作用都恢复了；对于其他人来说，只有侧面另一智能合同的调用的效果被恢复。这些违规行为会影响合同的安全性。
无Gas发送 | 当用户向合同发送以太时，可能出现没有gas的异常。
类型转换 | 编译器可以进行某种类型检查，但有不检查类型的情况，可导致意想不到的行为。
可重入性 | 回退机制可能允许非递归函数在终止前重新输入，它的终止可能导致循环消耗所有的gas ("Dao Attack” 臭名昭著利用了此漏洞)。
保守秘密 | 将字段声明为私有并不保证其保密性，因为 区块链是公开的，交易的内容是可察的。可能需要使用加密技术来保护秘密。
不可变修改性带来的问题 | 包括当在合同有漏洞，且并没有直接的方法来修补它时，无法更改部署的合同。（比如以太坊被盗后只有通过硬分叉来否认了以太坊失窃的后果）
交易可能丢失以太坊 |  以太发送到不使用地址后将会永远丢失，而且没有办法检测这个地址是否为不被使用的地址。
堆栈大小限制 |  调用堆栈会由1024帧和另一个调用绑定引发异常。（以太坊在2016年10月的硬分叉后已经解决了这个漏洞）
不可预知性 | 在向网络发送一笔交易时，并不能确保合同的状态是实际执行时的合同状态。此外，矿工将合同分组成块时不需要遵守的原有交易秩序。这导致黑客可以攻击“排队交易”这个漏洞。
随机性生成 | 一个矿工可以用他的程序块来伪造随机数的结果，冒充原有结果发送给用户。例如，彩票、游戏等这严重欺骗了使用者。
时间约束 | 时间约束可以用来确定当前状态应用程序允许进行什么操作。所以如果一个矿工在合同上持有股份，那么他可以通过选择一个合适的时间戳来获得额外收入。
...
合同可以直接接收以太坊 | 直接接收以太的合同需要实现一个回退函数的功能用来接收以太坊，否则函数将会产生异常并退回以太坊。如果退回功能没有实现程序具备警报功能，这样程序员就可以发现并人工的去做退回操作。
回退功能 | 合同具备这样一个保障功能，它能在花费不超过2300 gas的条件下退回接收的以太坊。我们的自动测试程序员已经在花费不到2300 gas的条件下完成这个回退测试。
嵌入和开发 | 当调用另一个合同时，被调用的契约可以通过它的函数改变调用契约的状态变量。QUANTSTAMP可以检查调用外部函数后目前合同状态变量的变化，可嵌入开发能做到这点实属不易。 
隐含声明 | 函数中明确的所有变量是指在整个函数的范围内的所有变量。这些现在变量处于整个函数初始化的默认值。编写恶意码会篡改函数中这些具有默认值的变量。当这种情况发生时，我们将会向用户发送警报。 
交易所属人 | 当检查tx.origin时可以立马停止该地址上进行的交易并得到该地址信息。如果智能合同必须要求tx.origin 的所有者操作，则黑客不能通过攻击钱包盗取所有的资金。因为在这这种情况下tx.origin仅仅具备钱包的地址的功能并不能进行转出。 
Gas传输 | 这是一个非常危险的功能称为地址索取费用功能。该功能给出gas接收合同，可以通过支付高额gas实现更多功能。这是一个有待进一步深入探讨的问题。
